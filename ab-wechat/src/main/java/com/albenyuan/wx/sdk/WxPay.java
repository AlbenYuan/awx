...
//        }
     ...
     * @return
     */
    public Map<String, String> buildOrder(String prepayId) throws Exception {
        Map<String, String> order = new HashMap<String, String>();
        String appIdKey = "appid";
        String nonceStrKey = "noncestr";
        String timeStampKey = "timestamp";
        String signKey = "sign";
        switch (config.getTradeScene()) {
            case APP:
                order.put("partnerid", config.getMchID());
                order.put("prepayid", prepayId);
            ...
            case JSAPI:
                ...
                order.put("package", "prepay_id=" + prepayId);
                signKey = "paySign";
                break;
        }
        order.put(appIdKey, config.getAppID());
        order.put(nonceStrKey, WxPayUtil.generateNonceStr());
        order.put(timeStampKey, (System.currentTimeMillis() / 1000) + "");
        String sign = WxPayUtil.generateSignature(order, this.config.getKey());
        order.put(signKey, sign);
        return order;
    }
